...
function insertar_persona($conexion, $datos) {
    ...
    $sql_persona = "INSERT INTO persona (nombres, ap_pat, ap_mat, fecha_nac, ci, direccion, password_hash, id_rol_usuario) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
function insertar_cuenta_bancaria($conexion, $id_persona, $tipo_cuenta) {
    $sql_cuenta = "INSERT INTO cuentabancaria (id_tipo_cuenta, id_persona) 
                   VALUES (?, ?)";
    ...
    // Asociar los parámetros para bind_param
    $stmt->bind_param("ii", $tipo_cuenta, $id_persona);
...
try {
    ...
    insertar_cuenta_bancaria($conexion, $id_persona, $datos_limpios['tipo_cuenta']);
